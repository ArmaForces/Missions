...
if (!hasInterface && !isServer || hasInterface && isServer) then {[this, 15, 150, 750, 0] execVM "M_scripts\civilians.sqf";};
...
params ["_logic","_number","_buildDist","_spawnDist","_type"];
_buildings = (nearestObjects [_logic, ["House", "Building"], _buildDist]);
_allAgents = [];
_playerDetect = false;
...
while {Madin_ai_spawning} do
{
	...
	{
		{
			if (!((vehicle _x) isKindOf "Air") && {(_x distance _logic) < _spawnDist})exitWith
			{
				_playerDetect = true;
				for "_i" from 1 to _number do
				{
					...
					while {_searching} do
					{
						...
						if ((count _allpositions) > 0)then
						{
							...
							_agent = createAgent [selectRandom (_arrCiv select _type), _pos, [], 0, "CAN_COLLIDE"];
	...
	{
		sleep 1;
		_playersNear = {(getPosATL _x distance _logic) < (_spawnDist + 100)} count allPlayers;
