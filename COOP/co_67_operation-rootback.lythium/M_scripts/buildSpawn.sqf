...
[this, 300, 50, 35, 6 ,2] execVM "M_scripts\buildSpawn.sqf";


// ==========
300 = promień liczony od lidera grupy, do których budynków będą spawnowały się jednostki
50 = ilość zespawnowanych jednostek
35 = ile m od budynku zostanie automatycznie usunięty z puli + zespawnowane stacjonarne jednostki
6 = ile jednostek w jednym patrolu
2 = ile razy maksymalnie z jednego budynku może zespawnować się patrol
_condition = skrypt który musi zwrócić {true}, by skrypt odpalił
...
_buildings = nearestObjects [_pos, ["House", "CBA_BuildingPos"], _distance];
_side = side _group;
_units = units _group;
...
private _grpType = [];
{
	...
	// Save single loadout per unit class
	if (_finder == -1) then {
...
toFixed 2;
{
	...
	// Check if we have spawn position for this building type
	{
		...
		if ((typeOf _building) == _buildingClass) exitWith {
			...
			_spawnBuildings pushBack [_building, getPosATL _building, _buildingPositons, _limit];
		};
	} forEach _knownBuildings;
} forEach _buildings;
_activation = _distance + (_minDist*2);
toFixed -1;

_location = nearestLocation [_pos, ""];
private _log = format ["[BUILDSPAWN] %1 budynków | %2 spawnerów | %3 ticketów | %4",count _spawnBuildings, _countPos, _tickets, _location];
systemChat _log;
[_log] call AF_fnc_serverLog;

waitUntil {
	sleep 1;
	private _near = (_pos nearEntities ["AllVehicles", _activation]);

	// return condition
	((_near findIf {isPlayer _x}) != -1)
	&& {_near call _condition}
};

private _spawnTargets = entities "HeadlessClient_F";
['[BUILDSPAWN] activated, spawning on targets', _spawnTargets] call AF_fnc_serverLog;

if (_spawnTargets isEqualTo []) then {
	['[BUILDSPAWN] WARNING, targets empty, spawning on server!'] call AF_fnc_serverLog;
	_spawnTargets = [2];
};

[_spawnBuildings,_minDist,_grpType,_side,_tickets,_limit,_activation,_pos,_patrol] remoteExec ["M_fnc_buildSpawn", selectRandom _spawnTargets];
