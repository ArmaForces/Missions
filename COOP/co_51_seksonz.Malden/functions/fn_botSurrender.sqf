...
if (isPlayer _unit
    ...
    || {_unit getVariable [QGVAR(surrenderInProgress), false]
    || {_unit getVariable ["ace_captives_isHandcuffed", false]}}}}
    ) exitWith {};

if ([objNull, "VIEW"] checkVisibility [eyePos _unit, eyePos _caller] >= 0.9) then {

    _unit setVariable [QGVAR(surrenderInProgress), true, true];
    ...
    if (random _courage <= 0.1 && alive _unit) then {
        ...
        [{speed vehicle _this == 0 && {isNull objectParent _this}}, {
            ...
            _unit setVariable [QGVAR(whWeapon), _whWeapon, true];
            _unit setVariable [QGVAR(whObject), _whObject, true];
            ...
            _unit setVariable [QGVAR(surrenderDone), true, true];
            _unit setVariable [QGVAR(surrenderInProgress), nil, true];

            //if (!(_unit getVariable [QGVAR(fleeScript), false])) then {
...
/*    } else {
        ...
        if (!(_unit getVariable [QGVAR(hasActionPunch), false]) && {!isPlayer _unit}) then {
                [_unit] call FUNC(botPunch);
            };
        */
    };
